...
```{r, echo=FALSE}
crosstalk::filter_slider('StaffFullPercent', 'Percent of healthcare staff fully vaccinated', final_data, 'StaffFullPercent', min=0, max=100, width='80%')

crosstalk::filter_slider('ResidentFullPercent', 'Percent of residents fully vaccinated', final_data, 'ResidentFullPercent', min=0, max=100, width='80%')
```
...
leaflet(final_data) |>
  ...
  leaflet::setView(lng = -110, lat = 48, zoom = 3) |>
...
```{r, echo=FALSE, asis=TRUE, warning=FALSE, message=FALSE, eval=FALSE}
...
library(plotly)
fig <- plot_ly(final_data, x = ~`ResidentFullPercent`, y = ~`Total Resident Covid Deaths Per 1,000 Residents`, color = ~`Current Healthcare Personnel Up to Date with Covid Vaccines`, colors=my_palette, type = 'scatter', mode = 'markers', text = ~PopupLabel, hoverinfo = 'text', marker = list(size = 10, opacity = 0.8)) 
